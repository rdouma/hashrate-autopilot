...
- **Income** = sum of coinbase-tx outputs credited to the payout address, captured via bitcoind RPC or Electrs against whatever Bitcoin node the operator has configured. Two observation modes (#170, see §11 of SPEC): the **full-history backfill** mode uses electrs's `blockchain.scripthash.get_history` to enumerate every tx that ever touched the address, then `blockchain.transaction.get` (verbose) to filter to coinbase txs and identify the vout(s) crediting the address - this captures historical Ocean payouts even after the operator has swept them off-address. The coinbase filter is the load-bearing correctness signal: Ocean is non-custodial, so genuine pool payouts are always coinbase outputs; non-coinbase deposits (operator self-sends, accidental transfers) are correctly rejected. The pre-1.7.5 mode (`listUnspent` / `scantxoutset`-based) returns only **currently unspent** outputs, which makes lifetime earnings track "what's still at the address," not "what was ever paid out" - asymmetric with the lifetime spend ledger and confusing to operators who reuse a payout address across before-and-after-installation periods.
