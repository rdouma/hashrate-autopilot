...
spot market, pointing purchased hashrate at a Datum-connected Ocean pool. The
Bitcoin + Datum stack can run on any always-on Linux host the operator
controls (bare metal, NAS, VPS, Umbrel, Start9, etc.); nothing in the
autopilot assumes a particular platform. Target sustained: 1 PH/s.
...
10. Payout observation is fully decoupled from Braiins: block rewards come from Ocean (via TIDES), sent to the BTC payout address that Datum encoded in coinbase. Observable via standard `bitcoind` RPC (`listreceivedbyaddress`, `listunspent`, or ZMQ + descriptor wallet) against any Bitcoin node the operator controls. Nothing Braiins-side is involved. ([datum_gateway README](https://raw.githubusercontent.com/OCEAN-xyz/datum_gateway/master/README.md), [Ocean DATUM origins](https://ocean.xyz/docs/datum))
...
- Ocean uses **TIDES** as its reward system. Ocean's marketing describes "miners receive their share directly from the coinbase transaction of found blocks," but this is **empirically not how operator payouts actually settle** (corrected 2026-06-17, per #240 and live evidence). What actually happens: Ocean's block rewards accumulate in a **P2SH pool wallet** (`37dvwZZoT3D7RXpTCpN2yKzMmNs2i2Fd1n`), and operator payouts are settled as **batched, multi-output, non-coinbase sweep transactions** broadcast from that wallet on Ocean's own cadence - mined into whatever block by whatever pool (observed: SBI Crypto, Foundry, etc.), NOT a coinbase and NOT necessarily an Ocean-mined block. `earnpay`'s `is_generation_txn` flag distinguishes coinbase-direct (`true`) from batched sweep (`false`); real operator payouts have been all `false`. The "non-custodial / coinbase IS the payout" framing is therefore misleading - Ocean holds accumulated balances between sweeps. ([ocean.xyz/docs/datum](https://ocean.xyz/docs/datum); [ocean.xyz - Origins of DATUM](https://ocean.xyz/docs/datum))
...
**Considered and discarded: historical `unpaid_sat` backfill.** The autopilot's Price chart has an `unpaid (sat)` right-axis series populated from `tick_metrics.ocean_unpaid_sat` - purely the cumulative result of per-tick polling. The chart shows nothing before the daemon started polling (or before migration 0053 added the column, whichever came later). Investigated 2026-05-29 whether Ocean's API could backfill the gap retroactively. Conclusion: not possible from primitives Ocean exposes.

- `statsnap` is current-snapshot only; no historical mode.
- `earnpay/<addr>/<from>/<to>` returns on-chain payouts in a date range (when balances crystallized and went to coinbase), not the unpaid trajectory between payouts. We already observe those payouts directly via Electrs / bitcoind on the payout address, so it adds nothing.
- `blocks` lists the pool blocks Ocean has found historically. Combined with operator hashrate over the period it tells us *which* blocks credited us, but to reconstruct the unpaid trajectory we'd need our `shares_in_window / current_tides_shares` at the time of each block, and Ocean does not expose that historically.
- The HTML stats page renders current values server-side; no archive mode.

A modeled estimate is theoretically possible - between any two consecutive on-chain payouts the unpaid balance rose from ~0 to ~`PAYOUT_THRESHOLD_SAT`, and you could interpolate using average hashrate × pool blocks/day × estimated share of pool over the period. But the result would be a model masquerading as Ocean's book; the chart's job is showing what Ocean actually had on file at each moment, not our reconstruction. Decision: leave the pre-polling gap visible. From the moment the daemon starts polling, the per-tick snapshot in `tick_metrics.ocean_unpaid_sat` is the authoritative record going forward. Reference community implementation (DeepSea-Dashboard) lives with the same limitation.
...
- **Income** = sum of transaction outputs credited to the payout address, captured via bitcoind RPC or Electrs against whatever Bitcoin node the operator has configured. Two observation modes (#170, see §11 of SPEC): the **full-history backfill** mode uses electrs's `blockchain.scripthash.get_history` to enumerate every tx that ever touched the address and identify the vout(s) crediting it - this captures historical Ocean payouts even after the operator has swept them off-address. **Correction (#240, 2026-06-17):** an earlier version of this mode filtered to *coinbase* txs on the assumption that "Ocean is non-custodial, so genuine pool payouts are always coinbase outputs." That assumption was wrong - Ocean settles operator payouts via batched non-coinbase sweep transactions (see the TIDES note above), so the coinbase filter silently dropped real payouts and was removed. The scanner now counts **any** tx paying the address. (Trade-off: an operator self-send / accidental transfer to the same address would be miscounted as income; acceptable given a dedicated payout address.) The pre-1.7.5 mode (`listUnspent` / `scantxoutset`-based) returns only **currently unspent** outputs, which makes lifetime earnings track "what's still at the address," not "what was ever paid out" - asymmetric with the lifetime spend ledger and confusing to operators who reuse a payout address across before-and-after-installation periods.
...
`"Next block"` comes from Ocean's `statsnap.estimated_payout_next_block` and is Ocean's own projection that, having crossed the threshold, the operator is queued to be paid in Ocean's next settlement. **Do not read it as "paid in the coinbase of the next block Ocean mines"** (an earlier version of this doc did - corrected 2026-06-17). Empirically Ocean settles operator payouts via **batched non-coinbase sweep transactions** from its P2SH pool wallet (see the TIDES note above), broadcast on Ocean's own cadence and mined into whatever block by whatever pool - not the coinbase of an Ocean-found block. So the on-chain settlement timing tracks Ocean's sweep schedule, not specifically when Ocean next wins a block. The exact sweep cadence (per-block, periodic, threshold-batched) is not fully nailed down and is an open question worth a live probe before asserting.
...
- **Dynamic home IP.** Braiins does not allow changing `dest_upstream.url` on a live bid (confirmed via OpenAPI - `SpotEditBidRequest` has no url/identity field). If the destination's public IP rotates after funding, the bid must be cancelled and a new one placed. Mitigation: DDNS hostname or static IP in front of the Datum Gateway. The autopilot itself ships a daemon-managed DDNS updater (No-IP / DuckDNS / generic dyndns2, see spec §8) so the operator does not need to lean on the router's firmware DDNS client.
