# Hashrate Autopilot - Specification (v2.19)

> Status: current, aligned with code through 2026-08-20.
>
...
- Braiins API reads (read-only token sufficient for all of these):
    ...
    - Per-bid delivery history: `GET /v1/spot/bid/delivery/{order_id}` (client implemented; not
      collected on any cadence today - per-bid delivery comes from `state_estimate` on `bid/current`)
...
- Braiins-derived: `braiins_total_deposited_sat`, `braiins_total_spent_sat`, `primary_bid_last_pause_reason`,
  `primary_bid_fee_paid_sat` (column exists but is always written null - its feed was part of the reverted #90
  work; revisit if per-bid fee accounting returns), `primary_bid_fee_rate_pct`. (#89)
...
- `max_overpay_vs_hashprice_sat_per_eh_day` *(default 2,000,000 sat/EH/day = 2,000 sat/PH/day; fresh
  ...
  configured ceiling. On `ocean_chain = bip110` (#363/#367) hashprice can never arrive (Ocean provides
  no API for that chain and none is derived - the chains' sats aren't comparable assets), so the
  refuse-to-trade rule is bypassed and the fixed `max_bid` is the only ceiling; cheap mode is inert
  for the same reason, and the Config UI renders both controls disabled with the reason.
...
Thresholds are individually tunable on Config -> Notifications. Defaults are tight so a fresh install catches problems fast; operators loosen as needed.
...
- `notify_on_pool_block_credit` - off-by-default INFO Telegram message at every TIDES credit (#117). Body contains block height, total reward, our credit in sat, our pool-share percentage at block time (as context, not the math derivation), and unpaid-total progress toward the 1,048,576-sat on-chain payout threshold. The credit number is Ocean's actual TIDES credit (#239, exactness fixed in #321): the primary source is the delta of `ocean_unpaid_sat` against the block's OWN noticing-time baseline - the deferral queue captures unpaid ~1 min after the block lands, before Ocean's ~4-min `user_hashrate` lag credits it, and the alert only fires once unpaid has risen past it - so the exact number needs no cross-restart memory. The previous-alert anchor remains a fallback, and the `~share_log_pct × reward` estimate (leading `~`) covers only genuine unknowns: multiple blocks credited in the same tick, or the payout-block failsafe where unpaid dropped instead of rose. When the block triggers an on-chain payout (detected via the unpaid-delta heuristic: noticed_unpaid + our_share - current_unpaid >= 65,536 sat), the title gains a "+ ON-CHAIN PAYOUT" suffix and a new body line reports the payout amount in sat and BTC (#171). Severity is INFO (no retry ladder, no inline ack button). The audible cue and the chart marker fire independently of this toggle.
...
- `notify_on_payout_confirmed` (#226, reworked in #323) - off-by-default INFO Telegram message, the enriched stage-2 follow-up to `payout_initiated`. Fires once per new settlement Ocean reports in its own payout ledger (`ocean_payouts`, fed by earnpay), and is **rail-aware**: it fires for BOTH on-chain and Lightning payouts and the body states which ("settled on-chain" vs "settled over Lightning"). This replaced the old on-chain-only `reward_events` source, which never confirmed a Lightning payout (the #323 bug). Idempotency via the per-row `enriched_alert` flag; the historical backfill is silently baselined on the first eligible tick so an upgrade doesn't replay old payouts. Txid is omitted from the body for privacy (the chart gem deep-links it).
...
- When `state.pool.consecutive_failures >= 3` (three consecutive ticks of the stratum TCP probe failing), the controller cancels all active bids to stop spend (#199). There is no point paying for hashrate that cannot reach the pool. The signal is deliberately the mandatory stratum probe, NOT the optional Datum stats API (`state.datum`): the protection must work when `datum_api_url` is unset, and a stats-API-only failure (healthy share path) must not cancel bids. When stratum recovers (consecutive_failures drops back to 0), the controller resumes normal operation and creates a new bid on the next tick. The `datum_unreachable` alert copy reflects this auto-cancel behavior.
...
IMPORTANT severity (11 marketplace + 4 solo = 15 total) - hard outages that need a phone alarm:
...
10. **Bidding held after bid churn** (#373) - `bid_churn_hold`. State-based, not time-based: mirrors the bid-guard's persisted CREATE hold of kind `churn`, which trips when bids are created and canceled back by the marketplace repeatedly with zero hashrate delivered. Bidding stays held (no CREATE proposals at all) until the operator presses **Resume bidding** on the Status page, which pairs the recovery; the body carries the guard's own `detail` line verbatim (untranslated daemon data).
11. **Marketplace blacklisted your pool** (#373) - `target_blacklisted`. State-based: mirrors the same persisted CREATE hold when its kind is `blacklist`, set from the marketplace's own "Target not allowed (blacklisted until …)" rejection. The hold auto-releases at `until_ms` and pairs the recovery then; the body states the expiry and carries the marketplace's rejection text verbatim. The operator may still resume early from the Status page.
...
14. **Bitaxe miner share-rejection high** - rolling-window rejection ratio >= `solo_share_rejection_threshold_pct` over `solo_share_rejection_window_minutes`. Re-arms once per window length; no recovery row.
15. **Bitaxe miner stratum URL drift** - device's reported `stratumURL` changed from the previously-observed value. Baselined silently on first poll so adding a device doesn't fire a spurious drift alert. No recovery row (new URL becomes the new baseline).

WARNING severity - soft warnings that can wait for the next dashboard glance:

16. **Beta-exit detected** - any active owned bid reports `fee_rate_pct > 0`.
...
- **Payout confirmed** (#226, reworked #323) - the enriched stage-2 alert; fires once per new settlement Ocean reports in `ocean_payouts` (earnpay), rail-aware so on-chain AND Lightning payouts both confirm (the body states which). Gate: `notify_on_payout_confirmed`. Severity INFO. Idempotency via the per-row `enriched_alert` flag, historical backfill silently baselined on the first eligible tick.
...
- Total funded in (snapshotted from `GET /account/balance`'s `total_deposited_sat`; individual deposits are
  listed separately via `GET /v1/account/transaction` for the deposit rows/markers. There is no funded-in
  manual override; the one manual knob is `historical_payouts_offset_sat` on the income side).
- Current Braiins wallet balance.
- Cumulative spend from filled bids. Scope is the `spent_scope` config knob: default `'account'` counts the
  whole account (autopilot + foreign bids, via the account-level spend service); `'autopilot'` restricts to
  ledger-owned bids (`SUM(amount_consumed_sat)` over `owned_bids`). The account path falls back to
  autopilot-only when its bid-list fetch fails, rather than falsely reporting zero.
...
- **Collected income comes from Ocean's own payout ledger (the `/v1/earnpay` endpoint), stored in
  ...
  **Timeline notes survive both row-replacing flows**: notes are keyed `payout:<id>` on the AUTOINCREMENT id, so
  ...
  and a slow trailing-window **incremental refresh** thereafter; upserts are idempotent on a per-address dedup key
  (txid for on-chain, synthetic ts+amount for Lightning). Ocean-unreachable is non-destructive: collected degrades
  to last-known.
- The legacy on-chain scanner (`payout-observer` -> `reward_events`, toggle `include_historical_payouts`, "Backfill
  now" button) still runs, but only as a silent corroboration/debug aid - it no longer feeds P&L. The manual
  `historical_payouts_offset_sat` is left untouched on upgrade; because earnpay now covers Lightning, an offset
  originally set to compensate for missed Lightning payouts may double-count, so the panel flags it for review.
- **Net result** as `/api/finance` computes it: `net = collected + historical_payouts_offset_sat +
  unpaid_ocean_earnings - spent`. Deliberately includes Ocean's not-yet-paid-out (unrealized) earnings and the
  operator's manual income offset; a missing `collected` is treated as 0, and net is null only when Ocean is
  unreachable. There is no separate realized-only net.
...
Six pages: **Status** (default), **Alerts**, **Timeline**, **Config**, **Setup** (first-run wizard only), and **Login**. The dashboard binds to the LAN only (`0.0.0.0:3010` by default; `HTTP_HOST=127.0.0.1` to restrict to loopback). Remote access is expected to go through a VPN / Tailscale perimeter; the dashboard has a shared-password second gate, not full auth.

### 12.1 Status page (top to bottom, default order)

The top-level blocks are drag-to-reorderable (#244 v3): a header **Rearrange** button reveals per-card grip handles; the order persists per device in localStorage. Default order: hero + next-action, range picker, stats bar, charts, service panels, active bids, P&L, last-tick proposals, BIP 110 card, Bitaxe miners. The list below follows that default.
...
- **Hero PRICE / DELIVERED card + run-mode toggle.** The large PRICE number is the **live current
  ...
  explicit and points at the AVG COST DELIVERED stats card for the post-hoc range-averaged
...
- **Stats bar** (configurable, #266). Operator-pickable tiles from a curated catalogue (24 entries spanning
  ...
  across browsers; defaults to the original six (UPTIME, AVG BRAIINS, AVG DATUM, AVG OCEAN, AVG COST
...
- **Hashrate chart.** Three series: `delivered (Braiins)` (amber), `received (Datum)` (emerald), `received (Ocean)` (blue). Target + floor as dashed horizontal references. Per-series rolling-mean smoothing via `braiins_hashrate_smoothing_minutes` and `datum_hashrate_smoothing_minutes`; Ocean is server-smoothed. Each chart's title carries an **expand / collapse toggle** (#105) that doubles the chart height for closer reading. Pool-block markers (one per Ocean-credited pool block) follow a precedence-ordered shape vocabulary (#115): own block (Ocean credited the coinbase to our payout address) -> **gold CROWN**; BIP 110-signalling pool block -> **yellow CUBE** (#94, Reduced Data Temporary Soft Fork; detection: `(version & 0xe0000000) === 0x20000000 && (version & 0x10) !== 0`, block-header version cached per `block_hash` via migration 0058); default pool block -> blue cube. Tooltip header label and color follow the same precedence (own > BIP 110 > default). Click opens the configured block explorer. A right-axis dropdown above the chart (#93, persisted to localStorage) selects one secondary series: `none` (default), `share_log %`, `network difficulty` (renders **difficulty-retarget markers** at every detected retarget tick - per-tick step > 0.5% with sustained-value check on the next non-null tick to filter spurious bucket-AVG detections; tooltip shows date, new difficulty, previous epoch's difficulty, and % change), `pool hashrate`, `rejection ratio (Braiins)`, `pool luck (24h)` / `pool luck (7d)` / `pool luck (30d)` (#92/#201, gap-based per-tick luck = `count_in_window / (pool_share × (window + elapsed) / 600)`; retarget markers also appear on the luck line (#174) with luck-before/luck-after values in the tooltip, since a difficulty retarget shifts pool_share and causes a discontinuous luck jump), `Bitaxe best difficulty` (#204, staircase line with trophy markers at each fleet all-time high share difficulty record; migration 0094), and - when solo mining is enabled - `Bitaxe hashrate`, `Bitaxe device count`, `Bitaxe max temp`. Difficulty retargets additionally render a **pickaxe icon** (Lucide `Pickaxe`, violet) at the top of the chart with a dashed vertical line (#175), always visible regardless of right-axis selection - the same top-of-chart treatment as block cubes/crowns. Hover/click opens the retarget tooltip.
- **Price chart.** Four always-on lines: `our bid` (amber), `fillable` (cyan, the controller's tracking anchor), `hashprice` (violet, dashed), `effective cap` (red, the per-tick `min(max_bid, hashprice + max_overpay_vs_hashprice)` ceiling, with a red gradient above the line marking the off-limits region). The cap line is reconstructed from per-tick history (`tick_metrics.max_bid_sat_per_eh_day` and `max_overpay_vs_hashprice_sat_per_eh_day`, the latter added in migration 0112, #312), so changing either knob today only moves the line from now forward - it never rewrites the historical ceiling. Bid-event markers on the amber line - emerald circle-plus CREATE, yellow dot EDIT_PRICE, sky gauge EDIT_SPEED, rose ban CANCEL (colors follow the `events.*` chart-color slots); clicking pins a detail panel with `fillable`, `overpay`, `hashprice`, cap inputs, effective cap at that tick, and a JSON export button. Per-range filtering: 3h-24h shows all four kinds; 1w drops EDIT_PRICE; 1m / 1y / all show none. See `CHART_RANGE_SPECS[r].showEventKinds` in `packages/shared/src/chart-ranges.ts`. Pool-block markers and difficulty-retarget markers are mirrored from the hashrate chart (#176) - the same top-of-chart cubes/crowns/BIP-110 shapes and pickaxe icons with dashed vertical lines, with identical tooltips. A right-axis dropdown (#93) selects one secondary series: `none` (default), `effective rate` (#90/#93), `block reward`, `BTC/USD`, `unpaid earnings`, **`paid earnings (lifetime)`** (#102, monotonically non-decreasing cumulative on-chain payouts to the configured address; per-tick `paid_total_sat` derived from `reward_events` via migration 0066), **`lifetime earnings (paid + unpaid)`** (the natural metric that survives payout cliffs - paid_total + ocean_unpaid), **`avg overpay (intent)`** / **`avg overpay (settled)`** (#164), `Bitaxe power (W)` (solo-gated), **`Braiins balance`** (#211, purple line from `tick_metrics.total_balance_sat` - the Braiins total wallet balance (available + blocked) polled every tick; zero-anchored right axis so the absolute level is visible at a glance; decreases as bids consume funds, jumps on deposits; migration 0095). When the right-axis is set to a step-event series (paid / unpaid / lifetime earnings), the chart renders **clickable dots** at each event - on-chain payout dots deep-link via `block_explorer_tx_url_template`, pool-block dots reuse the rich tooltip shape from the Hashrate chart (reward, our share, BIP 110 signal, explorer link) and additionally report the balance step the dot sits on - "unpaid balance at this step" (or lifetime earnings) with before, after, and the credited delta, taken from the same observed step the dot-placement scan used, so tooltip and picture can't disagree (batched Ocean credits share the combined step's numbers). **Payout gem markers** (#207, reworked #323): an **emerald gem** (Lucide `Gem`) at the top of the chart with a dashed vertical line marks every payout in Ocean's payout ledger (`ocean_payouts`/earnpay, served by `/api/payout-ledger`), covering **both on-chain and Lightning**. Clicking opens a pinnable tooltip with payout date and amount; on-chain payouts additionally show a block-explorer deep-link (keyed on `on_chain_txid`), while Lightning payouts are labelled "LIGHTNING PAYOUT" with no link (there's no on-chain tx to open). Because the source is earnpay rather than the on-chain scanner, node-less installs see gems too. A **purple dot** on the unpaid-earnings line marks the earlier tick where `ocean_unpaid_sat` drops >30% - the moment Ocean debits the balance to initiate the payout, bridging the visual gap between the unpaid line dropping and the gem appearing when the tx lands on-chain. **Deposit markers** (#211): a **purple fuel icon** (Lucide `Fuel`) at the top of the chart with a dashed vertical line marks every credited Braiins deposit (DEPOSIT_STATUS_CREDITED); clicking opens a pinnable tooltip with deposit amount, truncated tx_id, receiving address, timestamp, and a block-explorer deep-link. When the right-axis series is `total_balance_sat` (Braiins balance), a purple dot appears on the balance line at the step-up caused by the deposit, with a dotted connector line back to the fuel icon so the operator can visually trace which deposit caused which balance jump; hovering either the dot or the connector opens the same deposit tooltip. Visible regardless of right-axis selection. Data served by `/api/deposits` from the `braiins_deposits` table. **Lifecycle markers** (#287): a violet **arrow-left-right icon** (Lucide `arrow-left-right`, one shared glyph regardless of direction; moved off `power`, which now belongs to daemon-started boot markers) marks every MODE_CHANGE, a rose **circle-pause** every BID_PAUSED, an emerald **circle-play** every BID_RESUMED - top-of-chart glyphs with full-height dashed guide lines (these rows have no price anchor), **always visible at every zoom level** (they bypass the per-range kind filter: a mode change that explains a week-long gap must stay visible at the 1m zoom where the gap is noticed). Clicking pins the standard event tooltip; BID_PAUSED carries Braiins' `last_pause_reason` as the reason line. Legend chips for the three kinds appear only when such a marker is in view. **Daemon-started markers** (#320): an emerald **power icon** with a dashed guide line marks every daemon boot, always visible, with a pinned tooltip (build/version) and a "View in timeline" jump; bidirectional with the Timeline's boot rows via `focus_marker=boot:<id>`. **Idle-state background bands** (#287, both charts): a violet diagonal hatch spans every period the run mode was DRY_RUN or PAUSED - derived from per-tick `tick_metrics.run_mode` (exposed raw and worst-in-bucket PAUSED > DRY_RUN > LIVE via `/api/metrics`), so the bands are retroactive over all stored history; band edges snap to MODE_CHANGE event timestamps when one falls in the bracketing tick gap (tick-midpoint fallback for history without events). A rose counter-diagonal hatch spans every BID_PAUSED → BID_RESUMED pair (open-ended spans clamp to the data edge). Hovering a band names the state and duration. Band tints follow the `events.mode_change` / `events.bid_paused` color slots. **Alert condition bands (#316):** alerted condition spans render as hatched background bands with a dashed onset line and a hover tooltip (condition + source title + duration), all tinted by the single configurable `events.alert_condition` color slot (the span label carries the meaning, so one slot keeps the Config surface small). Hashrate-shaped conditions (below floor, zero hashrate, Bitaxe overheating) band the Hashrate chart; connectivity ones (DATUM / marketplace-API unreachable) band both; low wallet runway bands the price chart. **#374:** the hold and failure conditions join the same registry - `bid_churn_hold` and `target_blacklisted` band both charts because a create hold suppresses all trading, while `mutation_failed` (#372) bands the price chart only. `marketplace_empty` and `sustained_paused` are intentionally excluded because the fillable-null (#167) and bid-pause (#287) bands already cover them - but when one of their spans is the target of a Timeline "View on chart" jump, the price chart renders a beacon-only anchor (sonar rings + dashed guide at the jumped-to moment, no band) so the jump lands on something; the gating lives in the shared, unit-tested `conditionBandRenderMode` (#322). Spans are sourced from `GET /api/alert-spans`.
...
Dedicated `/history` route (#256 v2; the page is titled **Timeline** in the nav - the route path is unchanged)
replacing the older per-bid collapsible view. A flat table (fixed newest-first - the merged
infinite-scroll stream has no column sorting; the Excel export covers ad-hoc sorting) of every bid event - four bid-mutation kinds (CREATE_BID / EDIT_PRICE / EDIT_SPEED / CANCEL_BID) plus
...
carrying Lucide glyphs that mirror the rows, a full-text **Search** box (#342 - matches case-insensitively
across the effective/forward-filled bid id, the free-text reason, the row's personal note, and the
identifiers a row shows: block height/hash, IPs, deposit txids, config-change values; reason + note are
matched server-side via the event-notes join so hits in un-paged rows still surface, ANDed with the chips
and date range, the term a bound SQL param with LIKE-escaping), From / To date range via a custom locale-aware
date picker (replaces the browser-native `<input type=date>`, which always renders in the browser's
locale; the custom picker formats via `Intl.DateTimeFormat` in the dashboard's active locale),
denomination-aware `|Δ price| ≥ N` (input unit tracks the active TH/PH/EH toggle, converts to sat/PH/day
on the wire; the daemon rescales to sat/EH/day internally). Columns: When, Action, Bid (full id, no truncation), Fillable at event, Price before, Price
after, Δ price (green for downward, red for upward), Speed, Reason (#285 - the controller's
decision reason / mode transition / Braiins pause reason). Clicking a row slides in a detail drawer
(#285) with every field plus a "View on chart" jump that pans the Status price chart to the event,
scrolls the page to the chart, and pulses an amber focus ring around the marker for ~6 s. Any row also
takes a **personal note** (#336) - a free-text annotation persisted daemon-side, keyed to the event, that
feeds the Search index and rides into the Excel export. An alert-condition row's drawer breaks the outage
into threshold-before-firing / alert-fired / recovered / alert-duration / total-condition-time rows,
exact when the firing row recorded onset else estimated from current config with an `≈` marker + footnote
(#341, display-only). Every toolbar
filter persists per browser (sticky): the Action-kind chips, the Search term, date range and Δ-price filter, plus the
...
/ Price before / Price after / Δ price columns convert to the active denomination with the unit shown
once per column header on its own line (e.g. "Fillable" / "(sat/PH/day)"), the word "sat" rendered as
the Satoshi glyph; Speed carries the unit's own decimal precision. The detail drawer and the price-chart
event tooltip convert the same way. The free-text Reason column is generated by the daemon in canonical
sat/PH/day + PH/s; a client-side rewriter (`reasonUnits.ts`, tokenized + unit-tested) converts those
numeric tokens into the active denomination too, leaving the surrounding audit wording intact. The
...
**Excel export (#320).** The toolbar's **export** button streams the current (filtered) feed to a
`.xlsx` with a frozen header, autofilter, fixed column widths, and the denomination-aware values/headers
described above. Streaming (fflate, inline strings, row-by-row deflate) keeps memory flat regardless of
row count - no cap. A spinner shows while it works.

Server-side infinite-scroll pagination via a
`before_id` cursor; the window is the From/To date filters, independent of the Status page's range. SQL coalesces the bid id forward
...
**Alert condition rows (#316).** The Timeline feed also interleaves alerted condition spans as rows, so a sustained problem (delivered hashrate below floor, zero hashrate, DATUM or marketplace-API unreachable, low wallet runway, Bitaxe overheating) sits in the same chronological timeline as the bid activity around it. Each row carries the condition glyph + label tinted with the matching chart-band color, its duration (or "ongoing"), and the source alert body in the Reason column; the numeric bid columns are blank. Clicking a row opens a detail drawer (same pattern as bid rows); its "View on chart" button pans the Status price chart to the span start (the same `?at=` jump bid rows use). A second chip group in the toolbar ("Alerts") toggles each condition class on/off (client-side; all on by default). Spans come from `GET /api/alert-spans` (open/recovery alert pairs, orphan-bounded - see §13) and are bounded to the loaded bid-event range so an old alert can't float below a gap of not-yet-loaded bids. **#322:** a span closed by a real recovery alert renders a second row at the recovery moment (emerald check, "<condition> resolved", recovery body in the Reason column), toggled by the same condition chip and included in the Excel export; implicit closes (next episode / orphan bound) fabricate no row. The span payload carries `recovery_body` (null unless a real recovery closed it). Clicking a recovery row opens a recovery-flavored drawer (emerald resolved header, recovery body), and its "View on chart" pans to the band's CLOSING edge and beacons it there via `focus_span_edge=end` - opening rows keep jumping to the onset.

**Unified event log (#317).** Beyond bids and alerts, the Timeline feed also folds in the other notable account events as rows: on-chain payouts (`reward_events`), Braiins deposits (`/api/deposits`), pool blocks we found (`/api/ocean` `our_recent_blocks`, `found_by_us` only - non-ours context blocks stay chart-only), and public-IP changes (`/api/ip-changes`). Each renders with a Lucide glyph tinted to its chart-marker color, a short summary in the Reason column, and is toggled by an "Events" chip group. These rows are bound to the loaded bid range like alerts, so normal browsing stays recent; an older event is reached by clicking its chart marker. Every clickable chart marker's pinned tooltip carries a **"View in timeline"** link that navigates to `/history?focus=<kind>:<key>` (generic focus); the target row is force-shown regardless of the bid-range bind, scrolled to, and briefly highlighted. Bid-event markers keep their existing "Show in timeline" link (`?focus_event=`), and alert markers use `?focus_span=`.

**Complete log (#318).** The feed aims to be the single chronological log of every relevant event, whatever the source. Added on top of the above: **all pool blocks** (not just ours - others are context, ours flagged "found by us"); **difficulty retargets** (`/api/retargets`, derived server-side from `tick_metrics.network_difficulty` epochs, >0.5% jump = retarget); **`marketplace_empty` / `sustained_paused`** as log-only span rows (no chart band - #167/#287 already band those); **every remaining Telegram alert** as a point-alert row sourced from `/api/alerts` (excluding classes already represented by a span or a dedicated source, so future alert classes appear automatically); **config changes** (one row per changed field, recorded on save into `system_events`); and **daemon restarts** (a "daemon started" row per boot). Config/boot events come from `GET /api/system-events` (migration 0114). Reveal-from-chart: the retarget tooltip and the payout-initiated (unpaid-drop) dot carry links; the dot uses `?jump_ts=<ms>` (jump the log window to that time, no single-row highlight). Each new kind is a toggleable chip; volumes are low so all are shown by default. **#320:** daemon restarts now also render as a price-chart marker (always-visible power glyph, dashed guide line), so boot events jump bidirectionally like the other kinds - the chart tooltip's "View in timeline" (`?focus=boot:<id>`) and the Timeline row's "View on chart" (`?focus_marker=boot:<id>`). Config changes remain the only log kind with no chart representation.

### 12.4 Alerts page

Dedicated `/alerts` page (#100 / #109 / #134 / #139 / #153): event-grouped audit trail of every alert the daemon evaluated. Events render as collapsible cards grouped into two sections: **OPEN** (firing, not yet seen by the operator; the nav badge counts the unacknowledged IMPORTANT/WARNING subset - an open INFO alert shows in the section but not in the badge) and **Acknowledged and resolved** (one chronological bucket for both acked-not-recovered rows and recovery-paired rows, sorted newest-first by firing time; the merge prevents the bucket boundary from breaking chronological order when a recent IMPORTANT recovery lands after an older INFO ack). The per-card right-side pill keeps the state distinction visible per row: emerald `RESOLVED` vs slate `ACKNOWLEDGED · {age}`. Open cards render expanded by default; the merged bucket collapses to a header. A free-text search box filters across titles + bodies with hit-highlighting (#134). Sticky **Unacknowledged only** filter (persists per browser via localStorage). A **Mark all as seen (N)** bulk button next to the filter clears every unacked row in one click - server-side via `POST /api/alerts/acknowledge-all`. Telegram messages can also be acked in-place from the operator's phone via the inline-keyboard button (#109). A bottom-right **toast** appears in the dashboard the moment a new alert lands (#142), severity-coloured (red / amber / slate / emerald-for-resolved), with a 5 s auto-dismiss for INFO/recoveries and 15 s for the louder ones; clicking navigates to `/alerts`. The list paginates: a 50-row head page plus a "load older" button paging on a `before_created_at_ms` cursor (#121).
...
| 2.12    | 2026-07-05 | #323: earnpay-based, Lightning-inclusive payout tracking. P&L "collected" now sums Ocean's own payout ledger (`/v1/earnpay` -> new `ocean_payouts` table, migration 0116) instead of the on-chain-only `reward_events` scanner, so Lightning payouts count (they dropped `unpaid_sat` but never raised collected before, understating net P&L). `OceanPayoutsService` backfills full history on first run for an address and refreshes a trailing window on a slow cadence; collected is address-scoped and split into on-chain vs Lightning in the panel. Collected no longer needs an electrs/bitcoind node. The `payout_confirmed` Telegram alert is reworked to fire from `ocean_payouts` (rail-aware: on-chain AND Lightning), and the Price chart's payout gems come from `/api/payout-ledger` (Lightning gems have no explorer link). The on-chain scanner + `reward_events` + `include_historical_payouts` stay as a silent corroboration/debug aid; the manual `historical_payouts_offset_sat` is left untouched on upgrade but flagged as possibly double-counting now that earnpay covers Lightning. |
...
| 2.19    | 2026-08-20 | **Selectable Ocean chain + the BIP110 degradation story** (#363, #366, #367, #368; migrations 0122 `tick_metrics.ocean_chain`, 0123 `config.ocean_chain`). Since the 8/8/2026 chain split Ocean runs two chains with separate TIDES accounting and its JSON API covers only the mainstream one; Ocean confirmed (support, 2026-08-18) no BIP110 API exists, none is planned, and scraping is unsupported (a build-817 scraper was withdrawn on develop, never released). `config.ocean_chain` (`mainstream` default / `bip110`) is a declarative switch, live within one 60 s cache TTL. On `bip110`: a **chain-gated Ocean client** short-circuits every Ocean call without HTTP (stats/payouts -> null, blocks -> []), so the earnpay pipeline and deduced-payouts scanner are dormant; each tick stamps `tick_metrics.ocean_chain` and the deduced-payout drop-candidate SQL partitions by chain so flips can't mint phantom payouts. **P&L collected** derives from non-reorged `reward_events` via the operator's own node (labeled "collected (on-chain)"), Lightning reports as unknown (null, untrackable), net drops the structurally-null unpaid term, and the P&L rebuild / hard-reset routes re-run the on-chain address-history backfill (probe-before-wipe) instead of the earnpay fetch, surfacing errors instead of fake "0 payouts" successes. **No hashprice is served or derived** (Braiins spend is mainstream sats vs chain-local BIP110 sats - not comparable): the #28 dynamic cap's refuse-to-trade rule is bypassed (fixed `max_bid` is the only ceiling), cheap mode is inert, the status route + tick `getHashprice` return null (also suppressing the stale pre-flip cache value that fed bid-vs-hashprice / config_summary forever), and Config renders both controls disabled with the reason. **Dashboard degradation is marked, not hidden**: Ocean panel and P&L carry amber explanations, dead tiles render an amber "n/a" chip (tooltip says why), the tile picker and both chart right-axis dropdowns annotate dead entries "(n/a on BIP110)" while keeping them selectable, and the "received (Ocean)" / "hashprice" / "effective cap" legend chips dim with a pre-switch-history-only hover note - mainstream-era history keeps plotting, series end at the switch. A balance-check backend pointed at a BIP110-following node is required for earnings (Electrum recommended; bare `scantxoutset` loses swept payouts). All strings en/nl/es. README gains the "Mining on Ocean's BIP110 chain" chapter. |
...
| 2.15    | 2026-07-01 | Timeline polish + a controller safety fix. **History page renamed "Timeline"** in the nav (route `/history` unchanged). **Fully denomination-aware Timeline** (#320): the Fillable / Price before / Price after / Δ price columns, the detail drawer, the price-chart event tooltip, and the free-text reason (via a tokenized, unit-tested `reasonUnits.ts` rewriter) all follow the global sats/BTC/USD + TH/PH/EH toggles; column-header units sit on their own line with the Satoshi glyph for "sat". **Streaming Excel export** (#320): current filtered feed to `.xlsx` (fflate, inline strings, row-by-row deflate - flat memory, no cap), frozen header, autofilter, fixed column widths, denomination-aware values + unit-labelled headers (plain "sat" text where a font glyph can't render). **Live-tail "follow" toggle** and **bulk filter controls** (#320): per-group segmented all/none plus a global select-all / select-none pair. **Daemon-started price-chart marker** (#320): boot events render as always-visible power-glyph markers with a dashed guide and a pinned tooltip, jumping bidirectionally with the Timeline row (`focus=boot:<id>` / `focus_marker=boot:<id>`); config changes remain the only log kind with no chart representation. **Duplicate-bid guard** (#319): the CREATE branch of `decide.ts` no longer opens a second bid on a transient bid-list fetch blip - it requires `bids_fetch_ok` and zero active ledger bids before creating. No new migrations; no control-loop shape changes beyond the #319 guard. |
| 2.16    | 2026-07-02 | Code-vs-spec audit sweep (4 parallel reviews; ~60 findings; interview-resolved). **Behavior fixes:** datum-down auto-cancel keys on the mandatory stratum probe (`state.pool`), not the optional stats API - §9 updated with the rationale; dynamic-cap default codified at 2,000,000 sat/EH/day (fresh installs cap ON, §8); `handover_window_minutes` dropped (migration 0115) with §7.3 rewritten as retired-unshipped and the adopt/dismiss review flow moved to §12.5; Timeline Alerts/Events chips + follow are sticky and follow accelerates every merged source to 15 s (§12.3); `BHA_WALLET_RUNWAY_ALERT_DAYS` accepts fractional days. **Spec corrections to match code:** §5 uncollected inputs pruned (bid/speed, bars) and delivery marked implemented-but-unpolled; §7.1 PAUSED triggers (outage cancels, never pauses; PAUSE executes only in LIVE); §11 rewritten to the implemented accounting (spent_scope documented as the configurable it is (default account-wide), net includes unpaid earnings + manual offset, per-month ledger superseded by the range card, no receipt-time USD valuation); §12 intro six pages; §12.1 documents card reordering, last-tick proposals + Bitaxe cards, full right-axis option lists, real marker glyphs/colors, 24-tile catalogue; §12.3 column order, no column sorting, dashboard-locale date picker, sat/PH/day wire unit, drawer-first alert rows; §12.4 badge counts unacked IMPORTANT/WARNING only, load-older pagination; chart-color slots corrected to 26; `primary_bid_fee_paid_sat` noted always-null. |
| 2.18    | 2026-07-13 | Post-v1.17 feature run. **Current block-height StatsBar tile** (#335): names the pool/miner that mined the tip from the bundled mempool pools database (address + coinbase-tag match), gold crown when your payout address found it, BIP 110 / Ocean / grey cube otherwise, worker line for Ocean + own blocks, click-through to the configured explorer; hidden without a node. **Personal notes on Timeline events** (#336): any Timeline row takes a free-text note persisted daemon-side (keyed to the event), surfaced in the drawer and included in the Excel export. **Timeline full-text Search** (#342): the old "bid id" filter became a Search box matching case-insensitively across bid id (effective, forward-filled - so CREATE rows match by their eventual id), free-text reason, personal note, and row identifiers (block height/hash, IPs, deposit txids, config-change values); reason + note matched server-side via the event-notes join so hits in un-paged rows still surface, ANDed with chips + date range, the term a bound SQL param with LIKE-escaping. **P&L payout-store self-heal + hard reset** (#343): the earnpay full backfill now re-runs periodically and on every boot (was once-when-empty), so a partial fetch during an upgrade can't leave "collected" permanently short; the lifetime P&L card gains a **hard reset** control next to **rebuild** that wipes `ocean_payouts` + the Braiins spend cache and rebuilds both (fetch-before-delete; re-pulled payouts marked already-notified), and both controls report their result inline. **Alert-span drawer breakdown** (#341): the drawer splits a condition into threshold-before-firing / alert-fired / recovered / alert-duration / total-condition-time rows; exact when the firing row recorded onset (condition_started_at), else the threshold is estimated from current config with an `≈` marker + footnote (display-only, no stored data changed). Durations now round to the nearest unit instead of truncating (15m56s -> "16m", matching the recovery body). **DDNS test push** (#339) sends the daemon's detected public IP, not the dashboard request source. **Timeline drawer units** (#340) render muted with numbers right-aligned to a shared column edge. **Dependency bumps** (#344/#345): @fastify/static 9->10 (setHeaders callback now receives a FastifyReply), @fastify/cors 11.2->11.3, dev tooling. |
| 2.17    | 2026-07-03 | Post-audit polish shipped ahead of the release. **Recovery rows** (#322): every span closed by a real recovery alert renders a second Timeline row at the recovery moment (same condition chip, exported too), with a recovery-flavored drawer whose chart jump targets the band's closing edge (`focus_span_edge=end`); chart-less classes (marketplace_empty / sustained_paused) get a beacon-only price-chart anchor so their jumps land on something (`conditionBandRenderMode`, unit-tested). **Exact TIDES credit** (#321): `pool_block_credited` computes the credit from the block's own noticing-time unpaid baseline - restart-proof; the `~` estimate now marks only genuine unknowns. **Unpaid-line step dots** report before/after/credited for the balance step they sit on. **BIP 110 block table** renders BTC amounts with fixed 8 decimals. Screenshot set refreshed (order-history.png replaced by timeline.png). |
