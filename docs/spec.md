...
- **Stats bar** (configurable, #266). Operator-pickable tiles from a curated catalogue (~22 entries spanning
...
of every bid event (CREATE_BID / EDIT_PRICE / EDIT_SPEED / CANCEL) with toolbar filters: action-kind chips
...
on the wire). Columns: When, Bid (full id, no truncation), Action, Fillable at event, Price before, Price
