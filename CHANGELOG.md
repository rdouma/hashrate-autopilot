...
### `[Fix]` Telegram receiver respects the "Send messages to Telegram" master switch (#152)

Operator tapped Mark-as-seen on a Telegram alert; the message footer updated to acknowledged but the DB row on the primary install kept `acknowledged_at_ms = NULL`. Root cause: a second install configured with the same bot/chat token but with the "Send messages to Telegram" master switch off was still long-polling getUpdates. Telegram's getUpdates is single-consumer - whichever bot client polls first wins the update and confirms the offset; the others never see it. The secondary install's `markAcknowledged(id)` then silently no-op'd against its own DB (where the id resolved to a different alert or none at all - SQLite `UPDATE ... WHERE id=N` matches zero rows without throwing), but it still edited the Telegram message footer. Net effect: "acked in Telegram, unacked on primary's dashboard." Fix gates the `TelegramReceiver`'s `getCredentials()` on `notifications_muted`: when the master switch is off, `getCredentials()` returns null, the receiver loop sleeps, and getUpdates is not called. So Telegram disengagement is now end-to-end (inbound + outbound) rather than send-only. The receiver re-checks credentials each loop iteration (~15s) so toggling mute takes effect without a daemon restart.

### `[Fix]` Config search now indexes Solo miners + Display sections (#151)

Operator searched for "so" / "BitAxe" on the Config page and got hits only for Pool & Payout > Payout source + BTC price oracle > Price source - the Solo miners (Bitaxe / AxeOS) section was clearly visible on the same page but not in the search dropdown. Root cause: the cross-tab search indexer walks `TAB_SECTIONS[tab]` and matches IDs against the standard sections array first, then against `customSectionMeta` for ad-hoc components. Two Display-tab sections (`DisplaySettingsSection`, `SoloMinersSection`) were rendered inline outside that flow, so the indexer never saw them. Moved both into the standard `renderSection` switch (wrapped via `wrapHighlight` so post-search jumpTo scroll-and-flash still works), added `display-settings` and `solo-miners` IDs to `TAB_SECTIONS.display` (preserving their previous order at the top of the tab), added `customSectionMeta` entries with their titles + searchable field labels. Bitaxe / AxeOS now hit via title substring; Nerdaxe + ESP-Miner included as alias labels for the product-family search. No new translatable strings.
...
### `[UI]` Solo-mining: render `0.0 °C` VR temp as `—` (no sensor) (#149 follow-up)

Older Bitaxe Supra / Max board revisions don't have an I2C VR-temp sensor wired up; AxeOS returns `vrTemp: 0` instead of a missing-sensor flag, which then rendered as a visually-alarming `0.0 °C` on the Solo miners card next to two healthy 60 °C readings. Treat exactly `0.0 °C` for VR temp as "no sensor" and render `—` with a hover tooltip explaining why ("no VR temp sensor on this board (typical on older Bitaxe Supra / Max revisions)"). A running ASIC + VRM never legitimately reads 0 °C, so the false-positive risk is none. `tempClass` also stops applying the green/amber/red colour band when temp is 0, so the cell renders muted to match the missing-data semantics. The overheating alert was already unaffected (0 < ceiling means it never false-triggers on a 0-reporting sensor).
...
### `[Fix]` Solo-mining chart series: tick_at alignment + nearest-neighbor fallback (#149 follow-up)

Operator reported the new right-axis series stayed empty after picking `solo hashrate` / `solo device count` / `solo max temp` / `solo power (W)`, even though `solo_miner_samples` had 1,593 rows of real data. Root cause: `tick_metrics.tick_at` and `solo_miner_samples.tick_at` were offset by ~500-700 ms because each subsystem captured its own `Date.now()` during the controller tick. The chart's exact-match `byTick.get(p.tick_at)` lookup never hit. Two fixes: (1) the daemon now threads the canonical `r.state.tick_at` from the tick loop into `axeOSPoller.tick()` so future samples share the exact tick_at value tick_metrics uses, and (2) the chart projects the fleet series onto the x-axis using a nearest-neighbor join with a 15 s tolerance (linear two-pointer walk, O(n+m)), so the existing pre-fix samples render immediately and any future skew stays tolerable. Verified against the operator's running database: 1,593 samples spanning 9 h project cleanly across all four right-axis options.

### `[Feature]` Solo-mining chart right-axis series + purple-axis convention (#149, part 4/N)

Three new options on the Hashrate chart's right-axis selector (only listed when the solo-mining master toggle is on): **solo hashrate** (summed `hashrate_*_ghs` across the fleet, with the same 10m → 1m → 1h → instant fallback chain the Status card uses, formatted PH/TH/GH/s by magnitude), **solo device count** (count of devices where reachable AND hashrate > 0 — drops 3 → 2 when one falls off, recovers when it comes back), and **solo max temp** (max across devices of MAX(temp_c, vr_temp_c) — one overheating unit isn't masked by two cool ones). One new option on the Price chart's right-axis selector: **solo power (W)** (summed AxeOS `power` across the fleet, watts not kilowatts since a typical home Bitaxe fleet is 15-100W). Fed from a new daemon-side `GET /api/solo-miners/series?since=<ms>` route that aggregates `solo_miner_samples` GROUP BY tick_at (one SUM/MAX query, scales linearly with sample count). Dashboard query auto-bounded by the active chart range so a 3h view doesn't pull a week of samples. **Purple-axis colour convention**: per the operator's "only one right-axis line at a time, so always purple" rule, the Price chart's `effective_rate` series (was green `#34d399`) now renders in the same `#c084fc` purple as every other right-axis option on both charts. Single colour swap, no other behavioural change. NL + ES translations included.
...
Three pieces operator asked for after testing part 1. **Best Diff column** captures AxeOS's lifetime-best share difficulty (e.g. `149.53G`) per device on the Status Solo-miners card, plus a fleet-wide max in the summary footer (parsed by magnitude suffix so `149.53G` beats `225.68M` correctly). Hover shows the since-current-boot best (`bestSessionDiff`) as a tooltip. Captured per tick in a new pair of TEXT columns on `solo_miner_samples` (migration 0086) so the historical record matches what surfaces on the AxeOS Swarm screen. **Scan local network** button on Config → Solo miners → Add device fires a daemon-side /24 sweep (deduced from the daemon's non-loopback IPv4 interface), probes each IP for `/api/system/info` with a 200 ms timeout, and returns a confirmation dialog listing every AxeOS-shaped responder. Operator picks which to add, edits per-row labels (defaulted to `<ip> (<ASIC>)`), and confirms - already-saved IPs are rendered greyed-out and non-selectable. Mirrors the operator's mental model from AxeOS's own Swarm tab. Non-Bitaxe JSON responders on the same subnet (NAS REST endpoints etc) are filtered by structural shape check (`ASICModel` string OR `hashRate` number) so the suggestion list stays clean. **Hashrate fallback chain**: operator caught that three healthy units were reporting `-` for hashrate because part 1 only persisted the windowed `hashRate_1m/10m/1h` fields and the firmware was leaving them null. Migration 0087 adds `hashrate_instant_ghs` for the bare `hashRate` field and the dashboard now picks the most-useful available reading in priority order (10m → 1m → 1h → instant). NL + ES translations included.

### `[Feature]` Solo-mining monitoring core: Bitaxe / AxeOS device list + Status card (#149, part 1/N)

First slice of the #149 solo-mining feature. Operators with home Bitaxe / Nerdaxe / ESP-Miner units can now register the device IPs on Config → Display & Logging → Solo miners and see live hashrate, temperature, VR temperature, power draw, share-rejection rate, and uptime on a new Status-page card (with colour-coded temperature + rejection cells and a fleet-summary footer with total hashrate / total watts / J/TH efficiency / active-device count). Master toggle `solo_mining_enabled` defaults `false` so existing operators see no change until they opt in - with the toggle off the daemon does not poll AxeOS at all and the Status card renders nothing. Per-tick poller runs alongside the main control loop, fans out with `Promise.allSettled` (2 s per-device timeout) so one unreachable unit doesn't drag the rest, and persists samples to a new `solo_miner_samples` ring buffer for the chart series + delta-driven alerts that arrive in follow-up commits. Migration 0085 adds the `solo_miners` + `solo_miner_samples` tables and the master-toggle plus four alert-threshold columns in `config` (alert classes themselves land next). NL + ES translations included for every new dashboard string. Per-ASIC-model overheating defaults baked into the alert evaluator helper (BM1370 = 68 °C, BM1368/BM1366 = 70 °C, BM1397 = 75 °C, fallback 70 °C) with a single global Config override knob. Chart right-axis series and the four alert classes ship in follow-up commits.

### `[Infra]` /check-code follow-up: cleared the 19 deferred drift items (#148 continued)

Worked through the deferred items #148 catalogued from the earlier `/check-code` run. Three logical batches in one commit. **Snooze cleanup**: new migration 0084 drops the `snoozed_until_ms` column from `alerts` and normalises any historical `delivery_status='snoozed'` rows to `'gave_up'`; removed `'snoozed'` from the `AlertDeliveryStatus` union (daemon + dashboard); removed the field from repo `AlertRow` + insert + mock; renamed `markMutedOrSnoozed()` to `markMuted()` (its only live caller passed `'muted'` since snooze was retired in cc62951); dropped the `'snoozed'` branch from the dashboard `DeliveryBadge` render; corrected the `pruneOlderThan` JSDoc and the `telegram-receiver` callback-format JSDoc. **Spec §8 alignment** (docs-only): `wallet_runway_alert_days` typed as `nonNegativeNumber` (fractional allowed per `dc86586`); `tick_metrics_retention_days` + `decisions_eventful_retention_days` + new `alerts_retention_days` documented as `0 = keep forever` (matches code); `block_found_sound` enum corrected to `ocean-mining-found-block` (the actual filename, dropped the `-a-`); `chart_max_markers` (#123) and `spent_scope` added to §8 inventory; `datum_unreachable_alert_after_minutes` + `sustained_paused_alert_after_minutes` (#135) listed in §8 alongside the other outage thresholds; defaults that live in `APP_CONFIG_DEFAULTS` (not the Zod schema's `.default()`) annotated as such for `api_outage_alert_after_minutes` + `handover_window_minutes`; §9.1 INFO list split so "Ocean own-found block" and "On-chain payout received" are flagged as dashboard-only surfaces with no Telegram `event_class`. **Misc**: comments in `alert-evaluator.ts` referring to the old `LOUD` severity tier updated to `IMPORTANT`; module JSDoc updated to note `wallet_runway` is no longer stubbed; `notifications-test-event.ts` now accepts both `braiins_deposit` (dashboard tile alias) and `braiins_deposit_detected` (canonical event_class) as keys, so the canonical name works via `/api/notifications/test-event` too. No code-behaviour change in the daemon control loop; 304-test suite green; typecheck clean across all packages.

### `[Infra]` /check-code catch-up sweep: 3 high-severity drift items fixed (#148)

`/check-code` run against the freshly-swept specs caught three live-code items: (1) the `/alerts` page subtitle still read "sent, failed, muted, snoozed, or given-up" - user-visible stale text from the pre-snooze-removal era; dropped "snoozed" with NL/ES retranslations. (2) The dashboard's `StatusResponse.action_mode` wire type still listed `QUIET_HOURS | PENDING_CONFIRMATION | CONFIRMATION_TIMEOUT` from spec v1.1's retired state machine; narrowed to just `'NORMAL'` (matches the canonical `ActionMode` in `packages/shared`). (3) `operator_available` was dead plumbing end-to-end - HTTP endpoint at `/api/operator-available`, persistence column on `runtime_state`, dashboard API client method - no controller, service, dashboard page, or component reads or sets it. Spec v1.1 removed the action-mode state machine that backed it. Dropped the column via migration 0083, deleted the route file, removed the dashboard client method + StatusResponse field, cleaned up runtime_state repo + tests. Remaining 19 lower-severity drift items (snooze field/column remnants, schema-only config fields, low-acceptance TODO comment, dist leftovers) deferred for a dedicated cleanup session.

### `[UI]` Config Display: split number format and date layout into independent controls (#147)

The "Number & date format" dropdown bundled three concerns - thousands/decimal separators, date layout order/separators/12h-vs-24h, and month-name language - into a single Intl-locale string. An English-UI operator picking `1.234,56` European separators got Dutch month abbreviations leaking into the Status page's last-tick timestamp and other date surfaces, because the format-preference locale was wrongly threaded into date formatters (Alerts, Status, HashrateChart tooltips, PriceChart tooltips). Split into two dropdowns on Config → Display & Logging: **Number format** (separators only) and **Date layout** (US / European spaced 24h / slash DMY 24h / ISO / slash MDY 12h). Month names always follow the UI language picker (top-right) - English UI shows `Apr`, Dutch shows `apr`, Spanish shows `abr`, regardless of which date layout is picked. New `formatTimestamp({ uiLocale, layout })` hand-assembles the date string per the layout enum and pulls month names from the UI locale, so the two settings are finally orthogonal. One-shot localStorage migration: existing `braiins.displayLocale` → new `braiins.numberLocale` + `braiins.dateLayout` pair (en-US → us, en-GB / European locales → eu-spaced-24h, etc), legacy key cleaned up. NL ("Datumindeling" / "Getalindeling") and ES ("Formato de fecha" / "Formato numérico") translations included.
...
Live API probe (read-only token diagnostic) revealed two things wrong with #141. First, `tx_type` and `deposit_status` arrive as strings (`"ONCHAIN_TRANSACTION_TYPE_DEPOSIT"`, `"DEPOSIT_STATUS_CREDITED"`), not the OpenAPI-declared integer enums - so the watcher's `if (tx.tx_type !== 0) continue;` filter dropped every row and the `braiins_deposits` table stayed empty after each poll. Second, the endpoint only exposes deposits that have already reached the CREDITED state; the earlier "pending / confirming" stages aren't surfaced. Empirical evidence: a 300k-sat test deposit's `total_deposited_sat` ticked at 18:39 UTC, `available_balance_sat` ticked at 18:45 UTC, the endpoint included the row with timestamp 18:45:22. So the endpoint can't fire `_detected` even with the type bug fixed - by the time the row appears, the deposit is already available. Restructured: `_detected` now comes from the balance-delta detector in AlertEvaluator (restored from the pre-#141 design); `_available` and `_returned` come from the on-chain endpoint poll, with string comparisons (`tx_type === 'ONCHAIN_TRANSACTION_TYPE_DEPOSIT'`, `deposit_status === 'DEPOSIT_STATUS_CREDITED'`) replacing the broken integer ones. The two detectors are coordinated by signal source, not by lifecycle flag, and don't share state. Silent-baseline guard from #141 still absorbs the 9 historical deposits on first poll. No schema change; `braiins_deposits` table reused as-is.

### `[Feature]` Dashboard: bottom-right toast on every new alert (#142)

The /alerts nav-bar badge tells the operator "there's something to look at"; toasts tell them "something just happened, glance now." New `AlertToast` + `ToastStack` components mounted in `Layout`, fed from the existing `['alerts-head']` query (dropped the `unacknowledged_only` filter and bumped limit to 10 so a burst of arrivals between polls all surface as toasts). Detection compares the current-page max alert id against a localStorage-backed `lastSeenToastId`; first mount baselines silently, subsequent polls fire one toast per new id. Toasts auto-dismiss after 5s for INFO + recovery rows, 15s for WARNING + IMPORTANT; manual × button anytime. Click anywhere on a toast navigates to /alerts. Recovery rows render with the emerald RESOLVED pill, overriding the underlying severity so good news doesn't read red. Independent of `useBlockFoundSound` - the pool-block audio keeps its own poll path, decoupled from the toast layer. The server-computed `unacknowledged_high_severity_count` still drives the nav-bar badge unchanged. NL ("Sluiten") and ES ("Cerrar") translations for the new dismiss label included.

### `[Feature]` Restore Braiins deposit lifecycle: _detected + _available + _returned (#141)

The #132 pivot collapsed the three lifecycle events into a single `_detected` on the assumption that `total_deposited_sat` only ticks up once funds are spendable. Operator's 400k-sat test deposit empirically falsified that: `total_deposited_sat` jumped at 15:30 UTC (mempool / first-seen), `available_balance_sat` jumped at 15:42 UTC (Braiins-side clearing), 12 minutes apart. The `_available` event silently went unfired. Resurrected `BraiinsDepositWatcherService` (deleted in #132 commit `763d3f2`, ported to current i18n + style) - polls `/v1/account/transaction/on-chain` every 60s, fires `_detected` on first-see, `_available` when `deposit_status` crosses the threshold (placeholder `>= 3`, single-point-of-update if Braiins's empirical mapping differs), `_returned` when `return_tx_id` becomes non-null. Per-tx-id idempotency via the `braiins_deposits` table that #130 introduced and #132 left as dead weight - no schema change. Silent-baseline guard on first poll absorbs historical deposits (table empty + non-empty API response = backlog) so a daemon restart on a populated account doesn't flood Telegram. Removed `evaluateBraiinsDeposit()` from AlertEvaluator + the matching `lastNotifiedTotalDepositedSat` baseline state to avoid double-firing `_detected`. Added `braiins_deposit_available` + `braiins_deposit_returned` keys to the test-event route so each lifecycle leg is probeable via `/api/notifications/test-event`. Logs every observed status transition for empirical enum-mapping verification.
...
### `[Feature]` Alerts page: cursor-based pagination, "Load older" affordance (#121)

The /alerts page was capped at the most-recent 200 rows, hard-coded; on installs that retain alerts forever (the new #119 default) older rows would silently fall off the visible list. Reworked to cursor pagination keyed on `created_at` descending. Default page size 50; "Load older" button below the table fetches the next 50 and appends. Toggling the unacknowledged-only filter resets the loaded list to the first page (otherwise filtered + unfiltered rows would interleave). Server returns `total_count` and `has_more`; the header now shows "showing N of M". `over-fetch by limit+1` derives `has_more` cheaply without a second count query. Backend hard cap stays at 1000 so power-user API consumers can grab a big batch in one call.

### `[Fix]` Marker tooltips on price chart show historical overpay, not current (#120)

EDIT_PRICE / CREATE_BID / EDIT_SPEED / CANCEL_BID markers previously rendered the live config's `overpay_sat_per_eh_day` and `max_overpay_vs_hashprice_sat_per_eh_day` in the "MARKET AT THIS TICK" rows of their pinned tooltip - so a marker fired under overpay=300 read overpay=200 once the operator edited the value. Internally contradictory: the same tooltip's reconstruction line ("fillable X + overpay Y") was correct because it derived overpay from `price - fillable`. Migration 0077 adds `overpay_sat_per_eh_day` and `max_overpay_vs_hashprice_sat_per_eh_day` columns to `bid_events`; the controller's execute path now snapshots both at write time. Tooltip prefers the snapshot, falling back to live config only on legacy rows where the columns are NULL. JSON-copy payload also reflects the historical snapshot.

### `[Feature]` Test buttons on every per-event tile

Each event-class tile on Config -> Notifications now carries a yellow Test pill that fires a sample message of that exact event class through the saved Telegram bot. Operator: "I think all these messages should have a little test button on the right." New endpoint `POST /api/notifications/test-event` takes `{ event_class }` and sends a `[TEST]`-prefixed sample with the production-format severity emoji + bracket label, so operators can preview how the real message renders in their chat. Sample bodies are plausible synthetic data marked `[SAMPLE]` so they're never confused with live telemetry. Refuses with a clear error when no Telegram bot is configured.

### `[Feature]` Overpay-tuning helper card next to the Overpay field (#118)

The Overpay above fillable input on Config -> Strategy -> Pricing now carries a yellow "Recommended (last 7d)" card directly below it. The helper polls a new `GET /api/overpay-tuning` endpoint every 60 s; the daemon analyses the trailing 7 days of `tick_metrics`, classifies each row by regime (tracking / capped / under-fillable), and reports the p95 of `bid - fillable` over the tracking-regime rows as the recommendation - "you would have filled 95% of the time at this much overpay; anything above is paid premium that did not buy fill rate." Rounded up to 1 sat/PH/day, floored at the tick size. Displays the tracking-tick count + capped/under-fillable breakdown for transparency, the projected 30-day savings vs the operator's current value, and a one-click Apply that writes the recommendation back into the field (auto-save then persists it). When fewer than ~500 tracking ticks exist (~8 h of normal ticking), the helper shows "insufficient history" and the Apply button is disabled. When the recommendation matches the current value within 5%, Apply is also disabled and the helper says "your overpay already matches the recommendation". NL/ES translations included.
...
Operator spotted a sharp upward spike on the Price chart's `lifetime earnings (paid + unpaid)` line at the moment of an on-chain payout, with the line returning to baseline a bucket later. Diagnosed against the captured state.db: within a payout-transition bucket the operator's unpaid earnings drop mid-bucket to 0 (Ocean credits the on-chain payout), while `paid_total_sat` (aggregated as `MAX`) already absorbs the post-payout value. The bucketed lifetime sum was `MAX(paid)` + `AVG(unpaid)`, which double-counts the payout: the AVG smears the pre-zero positive values into the bucket even though by the end of the bucket they're zero. Replaced `AVG(ocean_unpaid_sat)` with a correlated subquery that picks the latest tick's unpaid value within each bucket - now both `paid_total_sat` and `ocean_unpaid_sat` use end-of-bucket semantics, and the sum is stable across the transition. Verified on the captured incident: 04:30-04:59 bucket on 2026-05-06 used to render lifetime=2,722,603 (a 381k-sat phantom overshoot); now renders 2,341,238 (matches surrounding buckets).
...
### `[UI]` Right-axis colour back to purple (#93)

Slate-300 (the previous neutral grey) read as washed-out white on the dark background and didn't pop against the other lines. Switched back to a purple - specifically `#c084fc` (purple-400, pink-leaning) rather than the previous `#ec4899` pink (which collided with the Price chart's max-bid red) or `#a78bfa` violet-400 (which would collide with the dashed hashprice line). Distinct from every existing palette entry; reads cleanly against slate.
...
### `[Fix]` Pool luck back to network-share, smoothed + sanity-floored (#92)

The previous switch to a self-referencing 7d baseline made the 7d luck mathematically forced to 1.00× regardless of actual block count - "1 block in 7 days" rendered as "1.00× lucky" which is nonsense. Reverted to the network-share-derived expectation (`expected = pool_share_of_network × 144 × windowDays`) but with two new guards: (1) the chart's pool-luck series uses a 30-tick trailing average of `pool_hashrate_ph` so single-tick delivery dips don't spike the line; (2) both surfaces reject the calc when the smoothed pool hashrate is implausibly small (< 1 EH/s vs Ocean's typical ~30 EH/s) - a collapsed estimator now produces a line gap rather than a fake luck spike. So 1 block in 7d at typical Ocean magnitudes now reads as ~0.02× (catastrophically unlucky) which matches operator intuition. Tooltips updated; NL + ES translations updated.

### `[Fix]` Pool luck no longer spikes 10× on single-tick delivery dips (#92)

The luck multiplier was using a Poisson expectation derived from `pool_hashrate_ph`, which is itself estimated as `user_hashrate_5m_ph / share_log_pct`. Both inputs jitter on single-tick delivery dips, and the ratio amplified the noise: when own delivery briefly dropped to ~0.05 PH/s, the estimated pool hashrate collapsed by ~10×, the expected-blocks count shrank with it, and luck spiked to ~10× even though no extra block was found. New baseline: `pool_blocks_7d / 7` - the pool's own recent week-long pace, derived purely from observed block counts (monotonic, no estimator). 24h luck = `blocks_24h / baseline`. The 7d luck against the same baseline reads 1.00× by definition; useful as a sanity-check column. Tooltips updated to explain the new math + that the baseline drifts with real pool growth (so we don't false-flag onboarding events as luck). NL + ES translations updated.

### `[Fix]` Just-executed banner showed 1000x-too-low price (#87)

The "Just raised bid: 48 sat/PH/day → 49 sat/PH/day" line on the dashboard's just-executed banner was reading 1000x too low - the actual prices were 48,500 / 49,500 sat/PH/day. Root cause in the relabelSummary regex chain: the daemon emits "48,500 → 49,500 sat/PH/day" (en-US locale, comma thousand-separator). The first arrow-pair regex correctly parsed and re-formatted via formatSatPerPhDay - but in nl-NL the formatter renders period as thousand-separator ("48.500"). The SECOND bare-rate regex then ran on the first regex's output, parsed "48.500" as 48.5 (period interpreted as decimal), and re-formatted to "48". Combined the arrow + bare regexes into a single pass per dimension so the bare alternative cannot re-process the arrow rewriter's output.

### `[Fix]` Age-gated DB fallback for BTC price at boot (#89)

The previous boot fix only kicked off a live oracle fetch; if the oracle was unreachable at boot, the first tick still wrote `btc_usd_price = null`. Now: if the live fetch fails, the daemon falls back to the most recent persisted price from `tick_metrics` - but only if it's within 15 minutes (covers transient oracle outages). Older fallbacks are explicitly rejected so a long-downtime restart (oracle was reachable at last shutdown but the daemon stayed off for hours/days) does NOT seed a stale price; it writes null and waits for the next live fetch to recover. Logs explain whichever path was taken.
...
Tooltips added to every meaningful row on the Ocean panel: ocean hashrate, hashprice (break-even), share log, unpaid, next block est., income/day est., next payout, found, our earnings, pool blocks 24h, pool blocks 7d, pool users, pool workers. The 24h/7d block tooltips explicitly explain the luck multiplier ("1,18× lucky" / "0,82× unlucky") - what 1.00× means, when wide variance is normal, when sustained drift is the actionable signal. NL + ES translations included for all 13 new tooltip strings.
