...
internal StartOS service-network defaults for Bitcoin RPC (`http://bitcoind.startos:8332`), Datum stats
(`http://datum.startos:7152`), and Electrs (`electrs.startos:50001`). Use those internal URLs inside Hashrate
Autopilot instead of the external StartOS proxy/interface URLs. For Datum on StartOS, Hashrate Autopilot reads the
dashboard HTML on port 7152 when `/umbrel-api` is not available. Bitcoin RPC still requires valid RPC username and
password credentials generated by Bitcoin Knots/Core. Override the fields from the setup wizard or Config page if
your local services use different ports or credentials.
