      ...
      },
      {
        ...
        fields: [
          ...
          },
          {
            ...
            kind: 'text',
            help: t`Optional. The HTTP API exposed by Datum Gateway. On StartOS use http://datum.startos:7152; avoid the external StartOS proxy URL. Leave empty to disable; the Datum panel will show "not configured".`,
            fullWidth: true,
          },
        ],
      },
        ...
        <label className="block sm:col-span-2">
          ...
          <span className="block text-xs text-slate-500 mt-1">
            <Trans>
              ...
              {' '}with RPC credentials generated by Bitcoin Knots/Core. Avoid the external
              StartOS service URL.
            </Trans>
          </span>
          {test.data && test.data.ok && (
  ...
  if (spec.kind === 'text') {
    ...
    return (
      <label className="block">
        ...
        {spec.help && <span className="block text-xs text-slate-500 mt-1">{spec.help}</span>}
        {/* #162/#StartOS: Datum exposes stats as /umbrel-api JSON on
            some builds and as dashboard HTML on the StartOS package. */}
